  ...
  (type-case ExprC a
    ...
    [letC (n rhs body)
          (interp body
                  ...
                  fds)]
    [unletC (n body)
          (interp body (unbind-var n env) fds)]
					
    [maxC (l r) (max (interp l env fds) (interp r env fds))]))

;; Which should be parenthesis and what should be brackets?
(define (unbind-var [n : symbol] [env : Env]) : Env
	(cond
		[(empty? env) env]
		[(symbol=? n (bind-name (first env))) (rest env)]
		[else (cons (first env) (unbind-var n (rest env)))]))
